...
async def pg_client():
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("ALTER TABLE telemetry DROP CONSTRAINT IF EXISTS telemetry_pkey"))
        await conn.execute(text("ALTER TABLE telemetry ADD PRIMARY KEY (id, measured_at)"))
        await conn.execute(
            text(
                """
                DO $$
