...
async def test_acceleration_pulse_sends_200A(mock_client):
    await _run_seq(SequenceEngine(mock_client), "ACCELERATION_PULSE")
    sent_currents = [
        c.args[1] for c in mock_client.send_command.call_args_list if c.args[0] == "SET_CURRENT"
    ]
    assert 200.0 in sent_currents


async def test_regen_braking_sends_negative_current(mock_client):
    await _run_seq(SequenceEngine(mock_client), "REGEN_BRAKING")
    sent_currents = [
        c.args[1] for c in mock_client.send_command.call_args_list if c.args[0] == "SET_CURRENT"
    ]
...
async def test_constant_discharge_sends_50A(mock_client):
    await _run_seq(SequenceEngine(mock_client), "CONSTANT_DISCHARGE")
    sent_currents = [
        c.args[1] for c in mock_client.send_command.call_args_list if c.args[0] == "SET_CURRENT"
    ]
...
async def test_capacity_test_sends_20A(mock_client):
    await _run_seq(SequenceEngine(mock_client), "CAPACITY_TEST")
    sent_currents = [
        c.args[1] for c in mock_client.send_command.call_args_list if c.args[0] == "SET_CURRENT"
    ]
